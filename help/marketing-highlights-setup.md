...
* パート B:Sales Qualifier エンドポイントとIDを収集します。
...
これらの資格情報により、[!DNL Marketo]はSales Qualifierに対して安全に認証できます。
...
1. **[!UICONTROL プロジェクトを編集]**&#x200B;を選択し、プロジェクトの名前を`Sales Qualifier Marketing Highlights`などの識別可能な名前に変更して、**[!UICONTROL 保存]**&#x200B;を選択します。
...
1. 認証タイプとして&#x200B;**[!UICONTROL OAuth サーバー間]**&#x200B;を選択し、**[!UICONTROL 次]**&#x200B;を選択します。
   ...
   **[!UICONTROL OAuth Server-to-Server]**&#x200B;を使用すると、[!DNL Marketo]はユーザーにログインを求めずに、そのサーバーから直接Sales Qualifier APIを呼び出すことができます。
...
* **サンドボックス名** - AEP サンドボックスの名前は、UIに表示される表示名ではなく、Sales Qualifier URL （`sname`値）に表示されるとまったく同じです。 小文字のURL値（例：`prod`）を使用します（`Prod`ではなく）。
...
**[!UICONTROL Call Webhook]** フローステップを、既存または新規のトリガースマートキャンペーンに追加します。 そのキャンペーンのスマートリストトリガーが、Sales Qualifierに送信するアクティビティを決定します。
