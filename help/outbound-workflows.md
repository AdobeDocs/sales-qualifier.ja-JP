...
管理者は、自分ではなくチームメイトにアウトバウンドワークフローを割り当てることができます。 ワークフローを起動する前に、ドロップダウンから「**[!UICONTROL ユーザーを割り当て]**」を選択し、チームメイトを選択します。
...
プロンプトを読むには、メールのタッチポイントを展開します。 プロンプトは、AIが各見込み客の電子メールを作成する際に、トーン、長さ、焦点、call to actionなどを誘導します。
...
送信ウィンドウ、タイムゾーン、日数、フォローアップおよび不在通知のルール、表示、および提案の頻度をデフォルトに戻すには、**[!UICONTROL デフォルト設定にリセット]**&#x200B;をオンにします。 アウトバウンドワークフロー名は変更されません。

管理者は、オプトアウトフッターをグローバルに設定します。これは、アウトバウンドワークフロー設定とは独立して、アウトバウンドメールに適用されます。 「[ グローバル電子メールオプトアウトの設定](integrations.md#configure-global-email-opt-out)」を参照してください。
...
生成されたメールでは、見込み客に対して保存された追加データが使用されます。これには、見込み客でインポートされた[ カスタムフィールド ](prospects.md#build-your-prospect-list)も含まれます。したがって、ドラフトはより関連性が高くなります。 サポートされるフィールドの固定リストはなく、新しいフィールドまたはカスタムフィールドは追加の設定なしで機能します。 AIは、このデータを関連する場合に使用します。見込み客に追加データがない場合は、メール生成はこれまでと同様に機能します。

また、見込み客の言語で、適切な挨拶、自然なフレーズ、適切なビジネストーンのメールを生成することもできます。 サポートされている言語には、フランス語、ドイツ語、スペイン語などがあります。
...
承認された電子メールは、アウトバウンドワークフローで選択した日数、送信ウィンドウ、アクティブ時間オプション、タイムゾーン設定に従って送信されます。 遅延がゼロのタッチポイントは、すぐに送信されます。他のすべてのタッチポイントは、設定された遅延に従います。 未承認の見込み客は&#x200B;**[!UICONTROL レビューの準備完了]**&#x200B;のままです。
...
共有されたアウトバウンドワークフローでは、チームメイトが自分の見込み客を登録できます。 一括アクションの使用時も含め、各自が登録した見込み客のみを管理または一時停止できます。 アウトバウンドワークフローのオーナーだけで、スケジュール、タイムゾーン、ケイデンスなどのプランレベルの設定を編集できます。 これらの設定はチームメイトに対して読み取り専用です。
