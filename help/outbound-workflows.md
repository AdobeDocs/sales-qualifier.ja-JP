...
エンゲージメントプランは、目標主導のアウトリーチケイデンスです。 目標とターゲティング基準を定義します。 そして、AIがマルチタッチケイデンスを提案し、見込み客一人ひとりにパーソナライズされたメールコンテンツを作成します。 ケイデンスをアクティブ化する前に、各メールを確認して承認します。

エンゲージメントプランは、次の4つの要素で構成されています。
...
| 概念 | 説明 |
| --- | --- |
| **エンゲージメントプラン** | 目標、ターゲティングフィルター、ケイデンス、設定によって定義された、再利用可能なアウトバウンドアクティビティ。 |
| **目標** | アウトリーチで達成すべきこと。 |
...
| **ターゲティングフィルター** | エンゲージメントプランを見込み客のサブセットに限定する条件。 |
| **ドラフト** | レビューの準備ができているものの承認されていない生成メール。 |
| **理由** | AIが使用したシグナルやデータソースなど、特定のメールの作成方法を説明します。 |
| **登録** | 見込み客のドラフトを承認する。これにより、ケイデンスがアクティブになり、エンゲージメントプランの送信ウィンドウ中に送信されるメールをキューに入れる。 |

次のセクションでは、エンゲージメントプランの作成、生成されたメールのレビュー、見込み客の承認、アウトバウンドワークフローの管理方法について説明します。

## エンゲージメントプランの作成

エンゲージメントプランウィザードには、**[!UICONTROL 目標]**、**[!UICONTROL ターゲティング]**、**[!UICONTROL 顧客接点の生成]**、**[!UICONTROL 設定]**、および&#x200B;**[!UICONTROL 見込み客の追加]**&#x200B;の5つのステップがあります。 目標が残りのステップを形成します。

1. 左側のナビゲーションで、**[!UICONTROL アウトバウンドワークフロー]**&#x200B;を選択します。
1. **[!UICONTROL 参照]** タブで、右上隅の「**[!UICONTROL + エンゲージメントプランを作成]**」を選択します。
...
**[!UICONTROL 設定]** ステップは、エンゲージメントプランの実行方法を制御します。

1. **[!UICONTROL エンゲージメントプラン名]**&#x200B;を確認し、必要に応じて変更します。
1. エンゲージメントプランごとに&#x200B;**[!UICONTROL 最大の見込み客]**&#x200B;で、エンゲージメントプランが一度に管理できる見込み客の最大数を確認します。
...
1. 各見込み客のタイムゾーンまたはエンゲージメントプラン **[!UICONTROL タイムゾーン]**&#x200B;を送信タイミングに使用するかどうかを選択します。 エンゲージメントプランのタイムゾーンを使用する場合は、オーディエンスと一致することを確認します。
1. **[!UICONTROL 権限]**&#x200B;で、**[!UICONTROL プライベート]** （デフォルト）を維持するか、**[!UICONTROL 全員と共有]**&#x200B;を選択します。 詳しくは、[ エンゲージメントプランの共有](#share-an-engagement-plan)を参照してください。
1. **[!UICONTROL 見込み客を保存して追加]**&#x200B;を選択します。

オプトアウトフッターは、管理者がグローバルに設定し、エンゲージメントプランの設定とは関係なくアウトバウンドメールに適用されます。 「[ グローバル電子メールオプトアウトの設定](integrations.md#configure-global-email-opt-out)」を参照してください。
...
生成が完了すると、エンゲージメントプランの詳細ビューでドラフトのレビューを求めるメッセージが表示されます。 Sales Qualifierでは、承認するまで電子メールは配信されません。

1. エンゲージメントプランの詳細ビューで、バナーの「**[!UICONTROL 下書きを確認]**」を選択します。
...
>[!TIP]
>
>文言やトーンの変更には、直接編集を使用します。 AI ]**で**[!UICONTROL &#x200B;生成を使用してメールを書き換えます。
...
承認された電子メールは、エンゲージメントプランで選択した日数、送信ウィンドウ、アクティブ時間オプション、タイムゾーンの設定に従って送信されます。 遅延がゼロのタッチポイントは待機なしで送信し、他のタッチポイントは設定された遅延に従います。 未承認の見込み客は&#x200B;**[!UICONTROL レビューの準備完了]**&#x200B;のままです。

## エンゲージメントプランの共有

各エンゲージメントプランには、**[!UICONTROL 権限]**&#x200B;の設定があります。 アウトバウンドワークフローは、デフォルトで&#x200B;**[!UICONTROL プライベート]**&#x200B;です。 所有者は、**[!UICONTROL 全員と共有]**&#x200B;を選択して、チームでエンゲージメントプランを利用できるようにすることができます。

>[!CAUTION]
>
>共有は永続的です。 エンゲージメントプランを&#x200B;**[!UICONTROL すべてのユーザーと共有]**&#x200B;に設定した後、それを&#x200B;**[!UICONTROL プライベート]**&#x200B;に変更することはできません。

共有されたエンゲージメントプランでは、チームメイトが自分の見込み客を登録することができます。 一括アクションの使用時も含め、各自が登録した見込み客のみを管理または一時停止できます。 エンゲージメントプランの所有者だけでも、スケジュール、タイムゾーン、ケイデンスなどのプランレベルの設定を編集できます。 これらの設定はチームメイトに対して読み取り専用です。
...
見込み客が不在のメッセージで返信すると、エンゲージメントプランが自動的に処理します。

* **自動再開**: デフォルトではオンです。 不在時の返信に返信日が含まれる場合、エンゲージメントプランはその日のケイデンスを再開します。 返却日が指定されていない場合、チームが設定できる再開後のバッファーの後に、エンゲージメントプランが再開されます。
* **手動オプション**：見込み客を手動で再開、一時停止、スキップすることもできます。 [既存のアウトバウンドワークフローの管理](#manage-existing-engagement-plans)を参照してください。

## 既存のアウトバウンドワークフローの管理

**[!UICONTROL アウトバウンドワークフロー]** ページの「**[!UICONTROL 参照]**」タブには、利用可能なすべてのエンゲージメントプランが一覧表示されます。 各カードには、目標、設定されたタッチポイント、パフォーマンス指標が表示されます。 このビューは、アウトバウンドワークフローのモニタリング、ドラフトのレビュー、見込み客の追加に使用できます。
...
* **予約の一時停止** – 見込み客が会議を予約すると、**[!UICONTROL 会議の予約の一時停止]**&#x200B;がさらにフォローアップを停止します。 [手順4: エンゲージメントプラン設定](#step-4-configure-engagement-plan-settings)を参照してください。

[ アウトバウンドパフォーマンス ](performance.md) ページで予約結果を追跡します。

## エンゲージメントプランのベストプラクティス

* **特定の目標を定義します。** ターゲティング、ケイデンス、電子メールはすべて、目標から生まれています。 エンゲージメントプランで達成したい成果を記述します。
...
* **編集ツールを変更と一致させます。** 文言やトーンに直接編集を使用したり、 再構築またはリフレームにAI ]**を使用して**[!UICONTROL &#x200B;生成します。
